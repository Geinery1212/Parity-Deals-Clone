...
export default function HomePage() {
  return <>
    <section style={{ background: 'radial-gradient(oklch(65% 0.29 25 / 0.3), oklch(73% 0.08 60 / 0.3), var(--background) 60%)' }} className="min-h-screen flex items-center justify-center text-center text-balance flex-col gap-8 px-4">
    ...
    <section id="pricing" className="px-8 py-16 bg-accent/6">
      <h2 className="text-4xl text-center text-balance font-semibold mb-8">Pricing software which pay for itself 20x over</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-screen-xl mx-auto">
        {
          subscriptionTiersInOrder.map((tier => {
            return <PricingCard key={tier.name} {...tier} />
          }))
        }
      </div>
    </section>
  ...
  return (
    <Card>
      ...
      <CardFooter className="flex flex-col gap-4 items-start">
        ...
        {canRemoveBranding && <Feature>Remove Parity Deals Clone Branding</Feature>}
